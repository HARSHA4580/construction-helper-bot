...
# ----------------------------
# Load glossary (convert keys to lowercase)
# ----------------------------
@st.cache_resource
def load_glossary():
...
glossary_keys = list(knowledge_base.keys())
...
def correct_spelling(text):
    """
    Correct spelling except for technical words like IS codes (IS 456), 
    grade notations (M25, Fe500), and numbers.
    """
    tokens = text.split()
    protected_tokens = []
    for token in tokens:
        if re.match(r"^(M\d+|Fe\d+|IS\d+|\d+)$", token, re.IGNORECASE):
            protected_tokens.append(token)  # keep as is
        else:
            protected_tokens.append(str(TextBlob(token).correct()))
    return " ".join(protected_tokens)


def find_best_match(user_input):
    """
    Finds the closest glossary term using fuzzy matching.
    """
    user_input_lower = user_input.lower()
    matches = difflib.get_close_matches(user_input_lower, glossary_keys, n=1, cutoff=0.75)
    if matches:
        return matches[0]
    return None
...
def is_construction_related(text):
    ...
    if any(word in text_lower for word in glossary_keys):
        return True
    if find_best_match(text):
        return True
    return False
...
def get_knowledge_answer(user_input):
    ...
    best_match = find_best_match(user_input_lower)
    if best_match:
...
with col2:
    st.image("https://img.icons8.com/color/96/construction-worker.png", width=120)
